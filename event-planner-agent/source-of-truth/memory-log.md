...
## 2026-09-19 First BRD problem-validation interview

- Evidence level: one detailed primary interview with a community-trip organizer. The findings are strong for this respondent but are not yet generalizable to all treasurers or groups.
- Primary problem: paper records can be lost or damaged, financial history is difficult to reconcile, and reports must be retyped into documents and WhatsApp. Missing shared evidence causes participants to suspect unpaid contributions or misuse of event funds, harms the committee, and may stop future activities.
- Primary success outcome: no unresolved suspicion or accusation of fund misuse caused by missing evidence, inconsistent totals, or opaque corrections.
- Planning boundary: destination voting, vendor research discussion, and itinerary coordination remain in WhatsApp. Committee members enter budget drafts in Wargakas without formal task assignment; only the chairperson finalizes the plan, capacity, budget, and participant contribution. Later committee edits create a revision that has no financial effect until re-finalized.
- Roles: chairperson is final authority and can exercise treasurer authority; optional vice-chairperson receives full chairperson authority only after explicit delegation and retains it until revoked; treasurer verifies participant payments and receives cash; committee members may edit shared financial records; participants have read-only visibility, may submit transfer evidence, and may report errors while the event is open.
- Access governance: chairperson and vice-chairperson manage membership and roles. The delegated vice-chairperson's actions are labelled in audit history. The chairperson may act alone when the treasurer is unavailable, including confirming payments and approving advances.
- Payment evidence: participants may upload transfer evidence, which remains pending and does not affect balance until the treasurer or acting chairperson confirms it. Committee members may upload on behalf of participants without the app. Only the treasurer, chairperson, or delegated acting chairperson receives and records cash; participant confirmation is not required for recorded cash.
- Participant transparency: participants see the live read-only ledger, payment states, income, expenses, sponsor funds, advances, refunds, balance, redacted receipts, and audit history. They may report errors before closing. WhatsApp arrears lists include participant names, paid amount, remaining amount, status, and deadline; pending evidence is labelled `Menunggu verifikasi`, not `Belum bayar`.
- Audit behavior: any committee member may edit another committee member's record, but every change stores actor, time, reason, and before/after values. Deletion is reversible soft deletion: cancelled transactions remain struck through and excluded from totals; restoration reactivates the same transaction without losing cancellation history.
- Expense behavior: expenses apply immediately with no approval threshold. Receipt is optional, but a no-receipt reason is mandatory. If the event balance is insufficient, the event balance is not allowed to go negative; the shortfall becomes a linked advance under the name of the person who provided the private money.
- Advances: a committee member, participant, or outside party may provide an advance. Advances are prioritized for repayment and must be reviewed by chairperson and treasurer together; the chairperson may approve alone if the treasurer is unavailable. The funder may convert all or part of an advance to sponsorship and reverse that conversion only before the first vendor payment.
- Sponsor and recalculation: sponsor funds may arrive before or after participant collection begins and remain in the event balance rather than triggering an immediate refund. Budget revisions, participant-count changes, and added activities recalculate targets after chairperson finalization; prior payments remain intact and any excess becomes event balance.
- Participant-specific costs: optional activities and personal extras are charged only to participants who receive them. Committee members record the charge and it applies immediately without participant approval; the participant may report an error before closing. The chairperson may use common balance as documented `Ditanggung saldo kebersamaan` charity for named participants, and the recipient remains visible.
- Refund cutoff: before the first vendor payment, a participant may cancel and receive a full refund. The first vendor payment creates a global `Dana sudah digunakan` cutoff. After it, a participant cannot cancel for refund and must use a replacement. The replacement pays the original participant directly, the event cash balance does not change, and only the chairperson approves the slot transfer.
- Capacity and deficit: the chairperson may change capacity. If fewer participants continue, the remaining participants pay the recalculated shortfall unless sponsor or participant-specific handling is chosen. The chairperson decides whether the trip continues or is cancelled and selects the method for resolving any closing deficit.
- Closing balance: advances and all other obligations are settled first. The chairperson decides whether remaining balance is carried to a future event, used for additional/charity expenses, or returned equally to every participant who travelled regardless of whether their contribution was personal, sponsored, or charity-supported.
- Event cancellation: after vendor payment, participants can receive only remaining cash and vendor refunds. The chairperson decides whether recoverable balance is returned equally or carried into the next event.
- Receipt privacy: the system masks sensitive data automatically, but the uploader must inspect and approve the redacted version. Original receipts remain committee-only; participants see only approved redacted images. The uploader may approve their own receipt.
- Reporting: reports are generated on demand from the same ledger. Participants and committee members may export PDF or DOCX; both embed redacted receipt images and authenticated receipt links. WhatsApp text is generated from the same data rather than retyped manually.
- Closing: an event cannot be locked while disputes, pending payment evidence, unresolved advances, missing required data, or unpaid obligations remain. Only the chairperson or delegated acting chairperson locks or reopens an event. Reopening requires an audited reason, preserves old report snapshots, and does not automatically notify participants. Participants cannot submit new error reports after locking.
- Follow-up: validate these rules with 4–6 more treasurers and 2–3 chairpersons before changing the implementation contract. Technical SIT and release work remain deferred during the discovery gate.

## 2026-09-20 Second BRD problem-validation interview (respondent 2)

- Evidence level: second detailed primary interview with a community-trip organizer who holds the money. Independence from respondent 1 is NOT yet confirmed: same destination (Dieng), same paper-notebook workflow, same Word/PDF/plain-text reporting. Verify the two respondents are not from the same social circle before treating any finding below as genuine n=2.
- Event narrated: Dieng trip, 17 August, 16 participants. Lodging at a host's house, so there was no accommodation vendor. Private vehicles; fuel and toll were paid from participant contributions.
- Workflow: WhatsApp discussion of destination, attractions, transport, snack and meal budget, then an expense estimate, then a baseline contribution announced on WhatsApp. Contributions were received in cash and kept tucked inside the same paper notebook used for recording. Every expense was written in the notebook and receipts were tucked into it. After the event the report was retyped in Word and shared to the group as a PDF plus a plain-text version, because older members cannot open PDFs.
- Committee members spent personal money when contributions were not yet fully collected; it was recorded and reimbursed from contributions once collection was sufficient. Personal money has never gone unreimbursed.
- Suspicion incident (Dieng): participants without the full transaction log assumed the chairperson or committee had not paid their own contribution. Participants asked historically how much was spent and where, because the report contained only rough aggregate expenses rather than itemized detail.
- Resolution: the treasurer phoned the individual participant and clarified verbally. The clarification was never written down or shared to the group, so it left no reusable trace. He also acknowledged that because the log lives in a paper notebook, some expenses may genuinely have been mis-logged or had their receipts lost.
- Evidence sharing: when participants asked for detail, receipts were photographed and sent to the WhatsApp group, unredacted and only on request. Payment was cash, so no transfer slips containing account numbers existed.
- Earlier event, second dispute type: a participant protested a sudden contribution increase. Cause: costs are per-head (accommodation and everything else grows with each extra person), the group's one or two main sponsors pledge a fixed rupiah amount rather than a share, and late join confirmations therefore raised the residual amount split between participants. The chairperson decided the increase. The respondent's own proposed fix was a cut-off for join confirmation.
- Sponsors: one or two main sponsors typically cover roughly 50 percent of trip expenses, but they state a fixed rupiah figure; the 50 percent is an outcome, not a commitment. A pledge is announced during discussion and may later be reduced or cancelled, which is also announced. Only money actually received is written in the book.
- Cancellation: a participant who had paid fell ill and the slot was taken by a replacement, who paid the original participant directly. When no replacement is found, the money is refunded. No vendor-payment cut-off exists in this community.
- Participant-specific costs: extra restaurant orders and extra attractions or rides are paid by those participants themselves and are kept entirely outside the book. The book records only what is communal for the group.
- Closing: the record is simply left as it is with no closing or locking step, but the remaining balance is announced to the group. The remainder is carried into the next activity; returning it happens only if there is no next activity or the group disbands, which is rare and effectively has not happened.
- Tool history: has never tried an app or a spreadsheet for event finances. Reason given: does not have such an application, and works from a phone.

### Comparison with respondent 1

- Converging across both respondents: financial suspicion is a real, recurring problem triggered by detail and evidence that were never shared; the chairperson sets and changes the contribution target; sponsors pledge fixed rupiah amounts; a replacement settles directly with the original participant while the event cash balance does not move; committee personal money acts as an advance repaid from the event cash; participants are entitled to see expense evidence; the remaining balance is announced and carried into the next activity; planning stays in WhatsApp; the report is retyped by hand and shared as PDF plus text; optional activities create participant-specific costs.
- Conflicting evidence, recorded rather than merged: the first-vendor-payment refund cut-off (respondent 1 yes; respondent 2 has no vendor and still refunds when no replacement is found); mandatory receipt redaction (respondent 1 yes; respondent 2 sends original receipts to the group); event locking and reopening (respondent 1 yes; respondent 2 has no closing concept at all); participant-specific charges entering the ledger (respondent 1 yes; respondent 2 keeps them outside it); using the surplus for charity (respondent 1 yes; respondent 2 never mentions it).
- New from respondent 2, not previously recorded: the accusation targets the committee not paying its own contribution, so committee members must appear as payers in the same participant list; an aggregate-only report triggers suspicion even though a report exists, so per-transaction detail is required; dispute resolution happened verbally and left no trace, so answers must attach to the transaction and be visible to everyone; the underlying data really is incomplete, so capture must happen at the moment of spending and an incomplete ledger must label its own gaps or transparency can increase suspicion; cash and records were stored in the same physical object; a sponsor pledge and a sponsor receipt are two different objects; the ledger boundary is communal money only; the target divisor is the actual active headcount rather than fixed capacity; participants need read access after the event has ended; the plain-text report exists because older participants cannot open PDFs.
- Weakened respondent-1 rules with no respondent-2 support and no observed failure case: advance-to-sponsorship conversion, advances blocking event closure, equal distribution of the surplus, charity use of the surplus, automatic receipt redaction, and the locking/reopening machinery.
- Implementation conflicts now supported by two respondents: sponsor locking after the first participant payment, independent replacement payments, and `targetPerPerson = need / participantCapacity`.
- Adoption: neither respondent has ever searched for or used any tool for event finances. The problem is repeatedly confirmed; willingness to switch tools has no supporting evidence at all. These are two separate hypotheses and the second one is currently unsupported.

## 2026-09-23 Session handoff: discovery gate still open

State at handoff, on branch `qa/cashbook-defect-sweep`, all changes uncommitted:

- Two problem-validation interviews are recorded. Respondent 1 (2026-09-19) and respondent 2 (2026-09-20). Their independence is still unverified, so no finding is `Validated` yet.
- Documentation-only edits are in `project-brief.md`, `decisions.md`, `backlog.md`, `memory-log.md`, and `data-model.md`. No application code was touched. Nothing was committed or pushed.
- Three implemented rules are contradicted by both respondents and are recorded as requirement gaps, not as authorization to change code: sponsor locking after the first participant payment, independent replacement payments, and dividing the target by `participantCapacity` instead of the actual active headcount.
- The strongest unresolved risk is adoption. The problem is confirmed twice; neither respondent has ever searched for or used any tool for event finances, so willingness to switch remains unsupported.

Next steps, in order:

1. Confirm respondent 2 is not from respondent 1's community or social circle.
2. Finish the respondent-2 topics not yet asked: delegation when the chairperson is unavailable, participant comprehension of the ledger, and error reporting.
